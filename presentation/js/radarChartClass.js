/////////////////////////////////////////////////////////
///////////////// The Radar Chart Class /////////////////
////////////// Adapted By Gordon Wade From //////////////
...
class RadarChartClass {
    ...
    initVis() {
        ...
        vis.g = vis.svg
            ...
            .attr(
                'transform',
                'translate(' + (vis.cfg.w / 2 + vis.cfg.margin.left) + ',' + (vis.cfg.h / 2 + vis.cfg.margin.top) + ')',
            );
    ...
    updateCfg() {
        ...
        vis.maxValue = Math.max(
            vis.cfg.maxValue,
            d3.max(vis.activityScores, function (i) {
                return d3.max(
                    i.map(function (o) {
                        return o.value;
                    }),
        ...
        (vis.allAxis = vis.activityScores[0].map(function (i, j) {
            return i.axis;
        })), //Names of each axis
        ...
        vis.rScale = d3.scaleLinear().range([0, vis.radius]).domain([0, vis.maxValue]);
    }
    ...
    updateVis() {
        ...
        vis.backgroundCircles = vis.axisGrid.selectAll('.gridCircle').data(d3.range(1, vis.cfg.levels + 1).reverse());

        vis.backgroundCircles
            .enter()
        ...
        vis.axisLabels = vis.axisGrid.selectAll('.axisLabel').data(d3.range(1, vis.cfg.levels + 1).reverse());
        ...
        vis.axis = vis.axisGrid.selectAll('.axis').data(vis.allAxis).enter().append('g').attr('class', 'axis');
        //Append the lines
        vis.axis
            .append('line')
            .attr('x1', 0)
        ...
        vis.blobAreas = vis.g.selectAll('.radarArea').data(vis.displayParks);
        ...
        vis.blobAreas
            ...
            .on('mouseover', function (event, d) {
                ...
                //Dim all blobs
                d3.selectAll('.radarArea').transition('dim').duration(200).style('fill-opacity', 0.1);
                //Bring back the hovered over blob
                d3.select(this).transition('highlight').duration(200).style('fill-opacity', 0.7);
            })
        ...
        //Create the outlines
        vis.blobOutlines = vis.g.selectAll('.radarStroke').data(vis.displayParks);
        ...
        vis.blobCircleWrapper = vis.g.selectAll('.radarCircleWrapper').data(vis.displayParks);

        vis.blobCircleWrapper.enter().append('g').attr('class', 'radarCircleWrapper');

        vis.blobCircleWrapper = vis.g.selectAll('.radarCircleWrapper').data(vis.displayParks);

        //Append a set of invisible circles on top for the mouseover pop-up
        vis.invisibleCircles = vis.blobCircleWrapper.selectAll('.radarInvisibleCircle').data(function (d, i) {
            return d.activityScores;
        });
        ...
        if (vis.cfg.legend == true) {
            vis.legendSquares = vis.legend.selectAll('.legend-square').data(vis.displayParks);
            ...
            vis.legendSquares
                ...
                .attr(
                    'x',
                    (d, i) =>
                        ((i % vis.cfg.legendColumns) * (vis.cfg.w + vis.cfg.margin.left + vis.cfg.margin.right)) /
                        vis.cfg.legendColumns,
                )
                .attr(
                    'y',
                    (d, i) =>
                        vis.cfg.margin.bottom / 2 +
                        Math.floor(i / vis.cfg.legendColumns) * vis.cfg.legendSquareSize * 1.25,
                )
                .attr('fill', (d, i) => vis.cfg.color(i));

            vis.legendLabels = vis.legend.selectAll('.legend-label').data(vis.displayParks);
            ...
            vis.legendLabels
                ...
                .attr(
                    ...
                    (d, i) =>
                        ((i % vis.cfg.legendColumns) * (vis.cfg.w + vis.cfg.margin.left + vis.cfg.margin.right)) /
                            vis.cfg.legendColumns +
                        vis.cfg.legendSquareSize * 1.25,
                )
                .attr(
                    'y',
                    (d, i) =>
                        vis.cfg.margin.bottom / 2 +
                        vis.cfg.legendSquareSize +
                        Math.floor(i / vis.cfg.legendColumns) * vis.cfg.legendSquareSize * 1.25,
                )
